...
def Analyse_data(text):
    ...
    for i in range(len(models)):
        ...
        prediction = model.predict(unseen_padded)[0]
        ...
        result = {
            ...
            'rating': int(prediction * 100) / 10
        }
        results.append(result)

    sorted_results = sorted(results, key=lambda x: x["rating"], reverse=True)
    highest_confidence = sorted_results[0]["rating"]
...
models = [
    tf.keras.models.load_model('G:\IMDB Sentimental Analysis\c1_snn_model_acc_0.75.h5'),      # Index 0
    tf.keras.models.load_model('G:\IMDB Sentimental Analysis\c1_cnn_model_acc_0.838.h5'),
    tf.keras.models.load_model('G:\IMDB Sentimental Analysis\c1_cnnop_model_acc_0.844.h5'),         # Index 3
    tf.keras.models.load_model('G:\IMDB Sentimental Analysis\c1_lstm_model_acc_0.852.h5'),  # Index 1
    tf.keras.models.load_model('G:\IMDB Sentimental Analysis\c1_bilstm_model_acc_0.853.h5'),  # Index 4       # Index 3
    tf.keras.models.load_model('G:\IMDB Sentimental Analysis\c1_cnn_lstm_model_acc_0.845.h5'),
    tf.keras.models.load_model('G:\IMDB Sentimental Analysis\c1_cnn_bilstm_model_acc_0.855.h5'),
    tf.keras.models.load_model('G:\IMDB Sentimental Analysis\c1_hybrid_model_acc_0.864.h5'),  # Index 2
]
...
# Removed the headers parameter since it's not needed here
...
def prediction_all():
    ...
    # Call the Analyse_data function and pass the text as an argument
    response = Analyse_data(text)
